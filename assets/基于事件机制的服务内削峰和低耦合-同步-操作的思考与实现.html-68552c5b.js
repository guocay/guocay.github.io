import{_ as n,X as s,Y as a,Z as t}from"./framework-9ecac9c7.js";const p={},e=t(`<h2 id="问题描述" tabindex="-1"><a class="header-anchor" href="#问题描述" aria-hidden="true">#</a> 问题描述</h2><blockquote><p>去年底在做项目的时候,我们有这样的一个需求,用户上传文档操作,首先将文档保存至服务器后再上传到ES数据库中解析文档.解析文档是一个很消耗时间的操作,我们不能等待解析完成后再对用户返回操作成功.</p></blockquote><blockquote><p>今年初在做项目的时候,我们有这样的一个需求,用户接收消息后,需要根据是否节假日且用户是否设置节假日自动回复对消息进行自动回复处理.且自动回复操作的成功与否要影响到消息是否接收.我们在分析需求后觉得这必须是一个事务性的服务.自动回复失败将不接收消息.</p></blockquote><h2 id="问题思考" tabindex="-1"><a class="header-anchor" href="#问题思考" aria-hidden="true">#</a> 问题思考</h2><ol><li>这两个需求正好是两个极端,一个是服务细节影响最终结果,另一个是服务细节不影响最终结果;</li><li>另外,我们在讨论需求后发现.在以后项目的升级过程中,在这些服务节点很有可能再增加其他细节(比如,接收消息后,除自动回复外,还需要做别的操作).这个时候,如果我们在函数中再追加别的代码必定会增加测试的难度和耦合度;</li><li>Spring Event是Spring Framework中一个很重要的功能之一.本质是依赖BeanFactory中声明的监听器,事件发布后在同一个线程中线性调用监听器的响应,既然在同一个线程中就可以实现异常的监控与控制.完美实现第二个业务场景;</li><li>而第一个业务场景相较于第二个,只需将监听器的响应放到新的线程中执行,即可实现.而scheduling也是Spring Framework中一个很重要的功能之一.可以实现函数级异步操作;</li></ol><h2 id="解决方案" tabindex="-1"><a class="header-anchor" href="#解决方案" aria-hidden="true">#</a> 解决方案</h2><p>事件与活动的包装</p><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code><span class="token doc-comment comment">/**
 * 平台内对于应用事件的包装
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">CustomPlugin</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">CustomActivity</span><span class="token punctuation">&gt;</span></span> <span class="token keyword">implements</span> <span class="token class-name">Plugin</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="ps" tabindex="-1"><a class="header-anchor" href="#ps" aria-hidden="true">#</a> PS</h2><ol><li>本功能的实现参考了Spring Plugin的设计理念,熟悉这个包可以更好的理解这部分内容;</li></ol>`,18),c=[e];function o(l,i){return s(),a("div",null,c)}const k=n(p,[["render",o],["__file","基于事件机制的服务内削峰和低耦合-同步-操作的思考与实现.html.vue"]]);export{k as default};
